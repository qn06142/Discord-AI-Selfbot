...
async def generate_response(instructions, history=None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with requests.post(endpoint, headers=headers, json=data, verify=False) as response:
    ...
    if (
        ...
        if modeltype == 0:
            ...
            if message.channel.id in active_channels:
                ...
                history = '\n'.join([': '.join((message.author.name, message.content)) async for message in message.channel.history(limit=100)][::-1])
